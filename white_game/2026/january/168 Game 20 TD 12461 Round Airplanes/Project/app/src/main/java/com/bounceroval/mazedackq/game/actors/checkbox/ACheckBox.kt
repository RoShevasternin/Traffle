package com.bounceroval.mazedackq.game.actors.checkbox
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            screen.game.soundUtil.apply { play(click, 0.7f) }
            return true
        }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.ONF -> Static.ACheckBoxStyle(
            default = screen.game.all.on,
            checked = screen.game.all.off,
        )
    }
