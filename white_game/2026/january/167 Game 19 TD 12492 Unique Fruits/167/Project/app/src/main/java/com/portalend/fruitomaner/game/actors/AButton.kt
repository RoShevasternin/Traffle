...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        ...
        Static.Type.Map -> Static.AButtonStyle(
            default  = TextureRegionDrawable(screen.game.all.map_def),
            pressed  = TextureRegionDrawable(screen.game.all.map_press),
            disabled = TextureRegionDrawable(screen.game.all.map_press),
        )
