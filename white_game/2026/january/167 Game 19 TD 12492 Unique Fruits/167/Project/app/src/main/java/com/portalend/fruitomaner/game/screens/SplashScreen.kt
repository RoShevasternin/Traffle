...
import com.portalend.fruitomaner.game.utils.region
...
class SplashScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private val loader by lazy { Image(game.splash.LOAD) }

    override fun show() {
        loadSplashAssets()
        setBackBackground(game.splash.BACKICH.region)
        super.show()
        loadAssets()
        collectProgress()
    }
    ...
    override fun AdvancedStage.addActorsOnStageUI() {
        addActor(loader)
        loader.apply {
            setBounds(307f, 641f, 254f, 260f)
            setOrigin(Align.center)
            addAction(Actions.forever(Actions.rotateBy(360f, 2f, Interpolation.smooth)))
        }
    }
