package com.royaltombsecrets.miniquizler.game.actors.checkbox
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.MUSIC -> Static.ACheckBoxStyle(
            default = screen.game.all.msc,
            checked = screen.game.all.msc_prs,
        )
        Static.Type.SOUND -> Static.ACheckBoxStyle(
            default = screen.game.all.snd,
            checked = screen.game.all.snd_prs,
        )
    }
