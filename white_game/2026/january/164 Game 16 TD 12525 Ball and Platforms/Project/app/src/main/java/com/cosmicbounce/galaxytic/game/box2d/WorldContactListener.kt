...
import com.cosmicbounce.galaxytic.game.utils.currentTimeMinus
...
class WorldContactListener: ContactListener {
    ...
    private fun playSound(bodyA: AbstractBody, bodyB: AbstractBody) {
        ...
        if (tmpArray.all { it.fixtureDef.isSensor.not() }) {
            when {
                tmpArray.any { it.id == BodyId.PLANFORMA || it.id == BodyId.STENA }
                -> if (currentTimeMinus(timeContactBorders) >= 120) {
                    soundUtil.apply { play(hitList.random()) }
