package com.skynebowow.runnerblue.game.actors.checkbox
...
import com.skynebowow.runnerblue.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.CHECK -> Static.ACheckBoxStyle(
            ...
            checked = TextureRegionDrawable(assets.CHECK),
        )
        Static.Type.SOUND -> Static.ACheckBoxStyle(
            default = TextureRegionDrawable(assets.SOUND_DEF),
            checked = TextureRegionDrawable(assets.SOUND_CHECK),
        )
    }
