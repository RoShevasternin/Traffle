package com.bunnypanny.eggcatch.game.actors.button
...
open class AButton(
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.BACK -> Static.AButtonStyle(
            default = TextureRegionDrawable(assets.BACK_DEF),
            pressed = TextureRegionDrawable(assets.BACK_PRESS),
            disabled = TextureRegionDrawable(assets.BACK_PRESS),
        )
    }
