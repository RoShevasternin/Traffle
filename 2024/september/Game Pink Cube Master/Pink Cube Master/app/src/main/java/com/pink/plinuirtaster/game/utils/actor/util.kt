package com.pink.plinuirtaster.game.utils.actor
...
fun Actor.setOnClickListener(soundUtil: SoundUtil? = null, block: (Actor) -> Unit) {
    addListener(object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            soundUtil?.apply { play(CLICK, 0.45f) }
...
fun Actor.animShowScreen(time: Float=0f, block: () -> Unit = {}) {
    addAction(Actions.sequence(
        Actions.moveTo(0f, 0f, time, Interpolation.fastSlow),
        Actions.run(block)
    ))
}
fun Actor.animHideScreen(time: Float=0f, block: () -> Unit = {}) {
    addAction(Actions.sequence(
        Actions.moveTo(-WIDTH_UI, 0f, time, Interpolation.pow3),
        Actions.run(block)
    ))
}
