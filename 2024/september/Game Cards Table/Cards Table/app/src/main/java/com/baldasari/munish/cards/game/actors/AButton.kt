...
open class AButton(
    ...
    private fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            screen.game.soundUtil.apply { play(magical_click, 0.45f) }
    ...
    private fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.Play -> Static.AButtonStyle(
            ...
            pressed  = TextureRegionDrawable(screen.game.all.play_prs),
            disabled = TextureRegionDrawable(screen.game.all.play_prs),
        )
        Static.Type.Rules -> Static.AButtonStyle(
            default  = TextureRegionDrawable(screen.game.all.rules_def),
            pressed  = TextureRegionDrawable(screen.game.all.rules_prs),
            disabled = TextureRegionDrawable(screen.game.all.rules_prs),
        )
        Static.Type.Sett -> Static.AButtonStyle(
            default  = TextureRegionDrawable(screen.game.all.sett_def),
            pressed  = TextureRegionDrawable(screen.game.all.sett_prs),
            disabled = TextureRegionDrawable(screen.game.all.sett_prs),
        )
        Static.Type.Restart -> Static.AButtonStyle(
            default  = TextureRegionDrawable(screen.game.all.restart_def),
            pressed  = TextureRegionDrawable(screen.game.all.restart_prs ),
            disabled = TextureRegionDrawable(screen.game.all.restart_prs ),
        )
        ...
        Static.Type.Menu -> Static.AButtonStyle(
            ...
            pressed  = TextureRegionDrawable(screen.game.all.menu_prs ),
            disabled = TextureRegionDrawable(screen.game.all.menu_prs ),
        )
    }
    ...
    object Static {
        ...
        enum class Type {
            Sett, Rules, Play, Back, Menu, Restart
        }
    }
