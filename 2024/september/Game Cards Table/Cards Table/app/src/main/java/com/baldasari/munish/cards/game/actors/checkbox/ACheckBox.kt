package com.baldasari.munish.cards.game.actors.checkbox
...
import com.baldasari.munish.cards.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchDown(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int): Boolean {
            ...
            if (isSound) screen.game.soundUtil.apply { play(magical_click, 0.45f) }
            return true
        }
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.GALKA -> Static.ACheckBoxStyle(
            default = TextureEmpty.region,
            checked = screen.game.all.galka.region,
        )
        Static.Type.PLAYER -> Static.ACheckBoxStyle(
            default = screen.game.all.pdef.region,
            checked = screen.game.all.pcheck.region,
        )
    }
