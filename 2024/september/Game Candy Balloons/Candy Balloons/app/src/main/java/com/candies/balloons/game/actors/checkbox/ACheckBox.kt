package com.candies.balloons.game.actors.checkbox
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.OFF_ON -> Static.ACheckBoxStyle(
            default = screen.game.all.off.region,
            checked = screen.game.all.on.region,
        )
        Static.Type.COLLECT -> Static.ACheckBoxStyle(
            default = screen.game.all.sw_coll_block.region,
            checked = screen.game.all.sw_coll.region,
        )
        Static.Type.TASKMASTER -> Static.ACheckBoxStyle(
            default = screen.game.all.perfect_block.region,
            checked = screen.game.all.perfect.region,
        )
    }
