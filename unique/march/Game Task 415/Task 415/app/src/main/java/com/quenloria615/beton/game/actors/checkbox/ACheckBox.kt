package com.quenloria615.beton.game.actors.checkbox
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Type) = when(type) {
        Type.NOTIFICATION -> ACheckBoxStyle(
            default = TextureRegionDrawable(gdxGame.assetsAll.NOTIFICATION_DEF),
            checked = TextureRegionDrawable(gdxGame.assetsAll.NOTIFICATION_CHECK),
        )
        Type.ON_OFF -> ACheckBoxStyle(
            default = TextureRegionDrawable(gdxGame.assetsAll.ON),
            checked = TextureRegionDrawable(gdxGame.assetsAll.OFF),
        )
    }
