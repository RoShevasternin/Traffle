...
import com.moonarcade.starlabyrinth.game.actors.SpinWheel
...
class MainFortuneWheelPanel(
    ...
    private val aRoulette = SpinWheel(screen)
    ...
    private fun spinRoulette() {
        if (gdxGame.ds_Gems.flow.value >= 5) {
            ...
            aRoulette.spin { winItem ->
                when(winItem.type) {
                    SpinWheel.WinType.GOLD -> gdxGame.ds_Gold.update { it + winItem.count }
                    SpinWheel.WinType.GEMS -> gdxGame.ds_Gems.update { it + winItem.count }
                }
                btnSpin.enable()
                btnSpin.resetEffect()
            }
