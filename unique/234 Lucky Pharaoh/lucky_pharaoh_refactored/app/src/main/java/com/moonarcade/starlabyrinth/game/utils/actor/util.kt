...
package com.moonarcade.starlabyrinth.game.utils.actor
...
fun Actor.setOnClickListener(
    soundUtil: SoundResourceHelper? = null,
    block: (Actor) -> Unit
) {
...
fun Actor.setBoundsScaled(sizeScaler: ScaleCalculator, x: Float, y: Float, width: Float, height: Float) {
    setBounds(sizeScaler.scaled(x), sizeScaler.scaled(y), sizeScaler.scaled(width), sizeScaler.scaled(height))
}

fun Actor.setBoundsScaled(sizeScaler: ScaleCalculator, position: Vector2, size: Vector2) {
    setBoundsScaled(sizeScaler, position.x, position.y, size.x, size.y)
}

fun Actor.setSizeScaled(sizeScaler: ScaleCalculator, width: Float, height: Float) {
