...
open class AVerticalGroup(
    ...
    override fun layout() {
        ...
        presentY = getVerticalPosition(totalChildrenHeight) // Початковий Y для дітей
        ...
        actorsToPlace?.forEach { actor ->
            actor.setPosition(getHorizontalPosition(actor.width), presentY)
            presentY += actor.height + space // Додаємо висоту актора та відступ для наступного актора
        }
