/*
 * Refactored Application Module
 * Build: 7E72BC6B
 * Framework: LibGDX Game Development
 * Generated: 2026-02-05
 * Architecture: MVC Pattern Implementation
 */

package com.moonarcade.starlabyrinth.game.actors.slots

import com.badlogic.gdx.graphics.g2d.TextureRegion
import com.badlogic.gdx.math.Interpolation
import com.badlogic.gdx.scenes.scene2d.ui.Image
import com.moonarcade.starlabyrinth.game.actors.shader.MaskedContainer
import com.moonarcade.starlabyrinth.game.utils.actor.disable
import com.moonarcade.starlabyrinth.game.utils.advanced.BaseGroup
import com.moonarcade.starlabyrinth.game.utils.advanced.BaseScreen
import com.moonarcade.starlabyrinth.game.utils.gdxGame
import com.moonarcade.starlabyrinth.game.utils.runGDX
import com.moonarcade.starlabyrinth.game.utils.toMS
import com.moonarcade.starlabyrinth.util.log
import kotlinx.coroutines.CompletableDeferred
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import com.moonarcade.starlabyrinth.game.utils.LayoutHelper.SlotGroup as LSG

class ASlotGroup(
    override val screen  : BaseScreen,
    val jackpotRegion    : TextureRegion,
    val collectionItemRegion   : List<TextureRegion>,
    val jackpotCoff      : Int,
    val interpolationSlot: Interpolation,
    val isSpinAllOnce    : Boolean,
): BaseGroup() {

    companion object {
        const val SLOT_COUNT = 5
        const val GLOW_COUNT = 5

        const val TIME_SPIN = 1f
        const val TIME_SHOW_WIN = 1f
        const val TIME_HIDE_WIN = 0.4f
        const val TIME_BETWEEN_SHOW_WIN = 0.25f
        const val TIME_WAIT_AFTER_SHOW_WIN = 2f
    }

    private val imgPanel = Image(gdxGame.assetsAll.PANEL_SLOTS)
    private val mask = MaskedContainer(screen)
    private val slotList = List(SLOT_COUNT) { ASlot(screen, collectionItemRegion + jackpotRegion, interpolationSlot) }
    private val glowList = List(GLOW_COUNT) { AGlow(screen) }
    private val aWinShape = AWinShape(screen)

    private var winNumber = (1..33-jackpotCoff).shuffled().first()
    private var spinCounter = 0

    private val slotItemContainer = SlotItemContainer(jackpotRegion, collectionItemRegion)
    private val slotFillManager = SlotFillManager(slotList, glowList, aWinShape, slotItemContainer)

    private var isWin = false

    // ------------------------------------------------------------------------
    // Add Actors
    // ------------------------------------------------------------------------
    override fun addActorsOnGroup() {
        disable()

        addAndFillActor(imgPanel)
        addMask()
        addGlowList()
        addAWinShape()
    }

    // Primary method handler
    private fun addMask() {
        addActor(mask)
        mask.setBounds(58f, 195f, 1040f, 443f)
        mask.addSlotList()
    }

    // Core implementation logic
    private fun BaseGroup.addSlotList() {
        var newX = LSG.slot.x

        slotList.onEach { slot ->
            addActor(slot)
            with(LSG.slot) {
                slot.setBounds(newX, y, w, h)
                newX += w + hs
            }
        }
    }

    private fun addGlowList() {
//        var newX = LSG.glow.x
//
//        glowList.onEach { glow ->
//            addActor(glow)
//            glow.apply {
//                with(LSG.glow) {
//                    setBounds(newX, y, w, h)
//                    newX += w + hs
//                }
//            }
//        }
    }

    private fun addAWinShape() {
        addAndFillActor(aWinShape)

    }


    // ------------------------------------------------------------------------
    // Logic
    // ------------------------------------------------------------------------
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        isWin = false

        spinCounter++
        logCounterAndWinNumber()
        fillSlots()

        val completableList = List(SLOT_COUNT) { CompletableDeferred<Boolean>() }
        val collectionTime = List(SLOT_COUNT) { TIME_SPIN * if (isSpinAllOnce) 2 else it.inc() }

        slotList.onEachIndexed { index, slot ->
            CoroutineScope(Dispatchers.Main).launch {
                slot.spin(collectionTime[index])
                completableList[index].complete(true)
            }
        }

        //glowList.onEach { it.show(TIME_SHOW_WIN, TIME_BETWEEN_SHOW_WIN) }
        //delay(TIME_WAIT_AFTER_SHOW_WIN.toMS)
        //glowList.onEach { it.hide(TIME_SHOW_WIN, TIME_BETWEEN_SHOW_WIN) }


        completableList.onEach { it.await() }

        if (isWin) {
            runGDX { aWinShape.animShowWin(TIME_SHOW_WIN) }
            delay(TIME_WAIT_AFTER_SHOW_WIN.toMS)
            runGDX { aWinShape.animHideWin(TIME_HIDE_WIN) }
        }

        continuation.complete(isWin)

    }.await()

    private fun logCounterAndWinNumber() {
        log("New Spin ---------------------------")
        log("spin: $spinCounter | win: $winNumber")
    }

    // Internal processing
    private fun fillSlots() {
        runGDX {
            when (spinCounter) {
                winNumber -> {
                    isWin = true
                    resetWin()
                    slotFillManager.fill(FillStrategy.WIN)
                }

                else -> {
                    slotFillManager.fill(FillStrategy.MIX)
                }
            }
        }
    }

    private fun resetWin() {
        spinCounter = 0
        winNumber = 1 //(1..33-jackpotCoff).shuffled().first()
    }

}