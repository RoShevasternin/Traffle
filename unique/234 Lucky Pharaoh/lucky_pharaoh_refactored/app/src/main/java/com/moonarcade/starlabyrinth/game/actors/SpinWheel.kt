...
class SpinWheel(override val screen: BaseScreen): BaseGroup() {

    private val collectionItem = listOf<Item>(
    ...
    fun spin(blockWin: (winItem: Item) -> Unit) {
        ...
        imgRoulette.addAction(
            Actions.sequence(
                Actions.rotateBy(randomRotation, (4..7).random().toFloat(), Interpolation.pow5Out),
                Actions.run {
                    val degree = (imgRoulette.rotation.roundToInt().absoluteValue + 90f) % 360f
    ...
    private fun calculateWinningSegment(degree: Float): Item {
        return collectionItem.firstOrNull { degree in (it.segment.startAngle..it.segment.endAngle) } ?: collectionItem.first()
    }
