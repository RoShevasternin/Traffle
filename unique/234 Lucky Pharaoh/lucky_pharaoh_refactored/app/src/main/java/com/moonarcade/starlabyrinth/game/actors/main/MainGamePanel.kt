...
class MainGamePanel(
    ...
    private fun addAPanelSelectLocation() {
        aPanelSelectLocation.apply {
            blockNext = { dataLocation ->
                this@MainGamePanel.screen.aBackground.animToNewTexture(gdxGame.assetsAll.collectionBackground[dataLocation.index], TIME_ANIM_SCREEN)
                gdxGame.presentBackground = gdxGame.assetsAll.collectionBackground[dataLocation.index]
            ...
            blockPlay = {
                PlayScreen.CURRENT_LOCATION_INDEX = aPanelSelectLocation.presentLocationIndex
                screen.hideScreen {
                    gdxGame.navigationManager.navigate(GameplayScreen::class.java.name, screen::class.java.name)
                }
            }
        }
    ...
    private fun addAPanelIncreaseJackpot() {
        ...
        aPanelIncreaseJackpot.blockUp = {
            ...
            val presentDataLocation = aPanelSelectLocation.presentDataLocation
            val presentLevelJackpot = gdxGame.ds_LevelJeckpot.flow.value[presentDataLocation.index]

            if (presentLevelJackpot < MAX_LEVEL_JACKPOT) {
                val priceUp = presentDataLocation.collectionPriceUp[presentLevelJackpot]

                if (gdxGame.ds_Gems.flow.value >= priceUp) {
                    gdxGame.ds_LevelJeckpot.update { list ->
                        val mList = list.toMutableList()
                        mList[presentDataLocation.index] = mList[presentDataLocation.index] + 1
                        mList
                    }
    ...
    private fun updateAPanelIncreaseJackpot() {
        ...
        if (presentLevelJackpot == MAX_LEVEL_JACKPOT) aPanelIncreaseJackpot.updateToMaxJackpot()
        else {
        ...
        aPanelLocation.updatePercent(presentLevelJackpot)
    }
    ...
    private fun checkIsLocationAvailable() {
        ...
        aLevelBlocked.updateLevel(presentDataLocation.levelAvailable)

        if (amountLevel >= presentDataLocation.levelAvailable) {
