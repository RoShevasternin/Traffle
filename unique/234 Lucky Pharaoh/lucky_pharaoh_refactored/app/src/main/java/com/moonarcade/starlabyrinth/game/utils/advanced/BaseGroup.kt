...
import com.moonarcade.starlabyrinth.util.OneTime
import com.moonarcade.starlabyrinth.util.cancelCoroutinesAll
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers

abstract class BaseGroup : WidgetGroup(), Disposable {

    abstract val screen: BaseScreen

    open val sizeScaler: ScaleCalculator = ScaleCalculator(ScaleCalculator.Axis.X)
    ...
    override fun draw(batch: Batch?, parentAlpha: Float) {
        preDrawArray.forEach { it.draw(parentAlpha * this@BaseGroup.color.a) }
        super.draw(batch, parentAlpha)
        postDrawArray.forEach { it.draw(parentAlpha * this@BaseGroup.color.a) }
    }
