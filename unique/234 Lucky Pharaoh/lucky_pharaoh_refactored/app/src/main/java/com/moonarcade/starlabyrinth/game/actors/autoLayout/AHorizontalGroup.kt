...
open class AHorizontalGroup(
    ...
    override fun layout() {
        ...
        presentX = getHorizontalPosition(totalChildrenWidth) // Початковий X для дітей
        ...
        actorsToPlace?.forEach { actor ->
            actor.setPosition(presentX, getVerticalPosition(actor.height))
            presentX += actor.width + space // Додаємо висоту актора та відступ для наступного актора
        }
    }
