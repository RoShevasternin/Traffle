...
import com.crystalpath.mystmazer.game.utils.ITEM_JACKPOT_INDEX

class Matrix5x3Handler(
    val matrix5x3        : Matrix5x3,
    val slotItemContainer: SlotItemContainer
) {
    ...
    fun generateSlot(slotIndex: Int): List<SlotItem> {
        ...
        slots[slotIndex].onEach { slotItemIndex ->
            val slotItem = when (slotItemIndex) {
                ITEM_JACKPOT_INDEX -> slotItemContainer.wild
