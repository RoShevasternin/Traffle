package com.crystalpath.mystmazer.game.utils

import com.crystalpath.mystmazer.game.data.DataAvatar
...
val GLOBAL_listDataLocation = listOf(
    DataLocation(0, 1000, 0, "Fortune Scroll Library",                        List(27) { it + 1 }, PosSize(89f, 78f, 692f, 692f)), // Available

    DataLocation(1, 5000,   5,  "Lucky Edo Garden",                      List(MAX_LEVEL_JACKPOT) { (it + 1) * 2 }, PosSize(-49f, 12f, 726f, 726f)),
    DataLocation(2, 10_000, 10, "Sakura Bliss Park",                         List(MAX_LEVEL_JACKPOT) { (it + 1) * 3 }, PosSize(-49f, 12f, 726f, 726f)),
    DataLocation(3, 20_000, 20, "Lucky Wild Zoo",                      List(MAX_LEVEL_JACKPOT) { (it + 1) * 4 }, PosSize(-49f, 12f, 726f, 726f)),
    DataLocation(4, 30_000, 30, "Golden Poker Lounge",                List(MAX_LEVEL_JACKPOT) { (it + 1) * 5 }, PosSize(-49f, 12f, 726f, 726f)),
    DataLocation(5, 40_000, 40, "Tiger Fortune Palace",                       List(MAX_LEVEL_JACKPOT) { (it + 1) * 6 }, PosSize(-49f, 12f, 726f, 726f)),
    DataLocation(6, 50_000, 50, "Golden Fruit Avenue",                        List(MAX_LEVEL_JACKPOT) { (it + 1) * 7 }, PosSize(-49f, 12f, 726f, 726f)),
)
