...
class AMainShop(
    ...
    private fun addAPanelSelectInapp() {
        ...
        aPanelSelectInapp.blockBuy = { price ->
            val buyIndex = when (price) {
                "1.99$" -> 0
                "2.99$" -> 1
                "9.99$" -> 2
                else -> 0
        ...
        gdxGame.activity.blockPurchase = { purchase ->
            when(purchase) {
                "100_coins" -> {
                    gdxGame.ds_Gold.update { it + 100 }
                }
                "1000_coins" -> {
                    gdxGame.ds_Gems.update { it + 1000 }
                }
                "10000_coins" -> {
                    gdxGame.ds_Gems.update { it + 10_000 }
                }
            }
        }
    }
    ...
    private fun buy(index: Int) {
        gdxGame.activity.apply {
            productDetailsList?.let { listDetails ->
                if (listDetails.isNotEmpty()) launchBilling(listDetails[index])
            }
        }
