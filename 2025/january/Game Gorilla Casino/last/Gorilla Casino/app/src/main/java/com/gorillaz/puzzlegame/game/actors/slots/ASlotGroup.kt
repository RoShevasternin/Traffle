...
//        }
    ...
    suspend fun spin() = CompletableDeferred<Boolean>().also { continuation ->
        ...
        runGDX { gdxGame.soundUtil.spin.sound.pause() }

        if (isWin) {
            runGDX {
                gdxGame.soundUtil.apply { play(win_slot) }
                aWinShape.animShowWin(TIME_SHOW_WIN)
            }
            delay(TIME_WAIT_AFTER_SHOW_WIN.toMS)
    ...
    private fun resetWin() {
        spinCounter = 0
        winNumber   = (1..33-jackpotCoff).shuffled().first()
    }
